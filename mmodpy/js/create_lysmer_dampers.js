...
var zminArr = new Array();
var zmaxArr = new Array();
...
allNodes.forEach(function(N) {
    ...
    if(N.z < zmin){
        zminArr.push(N.nid)
        baseNodes.push(sparseNodeArr[N.nid]);
    };
    
}, this)


var baseNodesChk = new Array();

var boundnodint = new Array();

for (i=0; i<baseNodes.length; i++)
{
    baseNodesChk[baseNodes[i]] = 1;
    
}


Message("finding solids on bottom boundary");
...
for(i=0;i<allSolids.length;i++)
{
    ...
    if( baseNodesChk[intn] == 1){NodSol[intn].push(allSolids[i].eid);}
...
function buildDampers(baseNodesArr){
...
var complete = false;
var lastCount = 0;
baseNodesArr.push(baseNodesArr[0]);

while(counter < baseNodesArr.length){
    var i = counter;
    ...
    if(!inCol && !firstcycle){
        ...
        var lastNx = allNodes[sparseNodeArr[baseNodesArr[i-1]]].x;
        var lastNy = allNodes[sparseNodeArr[baseNodesArr[i-1]]].y;

        // get x and y coord of current node
        var curNx = allNodes[sparseNodeArr[baseNodesArr[i]]].x;
        var curNy = allNodes[sparseNodeArr[baseNodesArr[i]]].y;
        ...
        if(dX < tolerance && dY < tolerance){
            ...
            tmpMainNodeCol.push(allNodes[sparseNodeArr[baseNodesArr[i-1]]].nid);
            inCol = true;
        }
        else if(inCol){
            ...
            var nodesToCut = tmpMainNodeCol.length;
                baseNodesArr.splice(0,nodesToCut)
